...
const setSelectedLanguage = useUIStore((state) => state.setSelectedLanguage); // Giả sử store đã có hàm này

useEffect(() => {
  if (watchedLanguage) {
    setSelectedLanguage(watchedLanguage);
  }
}, [watchedLanguage, setSelectedLanguage]);
  ...
  // Trong FloatingGenerateBox.tsx
const watchedEngine = form.watch('engine');

useEffect(() => {
  if (watchedEngine) {
    // Ép Store cập nhật ngay khi Engine trong Form thay đổi
    setSelectedEngine(watchedEngine);
    console.log("🛠 Đã đồng bộ Engine lên Store:", watchedEngine);
  }
